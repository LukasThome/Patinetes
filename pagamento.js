...
db.serialize(() => {
  db.run(
    "CREATE TABLE transacoes (id INTEGER PRIMARY KEY, userId TEXT, valorCobranca REAL, transacaoId TEXT)"
  );
});
...
db.serialize(() => {
  db.run(
    "CREATE TABLE cartoes_de_pagamento (id INTEGER PRIMARY KEY, userId TEXT, numeroCartao TEXT, nomeTitular TEXT, dataValidade TEXT, codigoSeguranca TEXT)"
  );
});
